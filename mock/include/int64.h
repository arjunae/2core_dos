...
static inline _int64_t int64_from_int32(int32_t x) { return (_int64_t){ (x < 0 ? -1 : 0), (uint32_t)x }; }
...
static inline _int64_t int64_mul_i64_i32(_int64_t x, int32_t y) {
	...
	uint32_t ypos = (y < 0)? (-y) : (y);
     	...
     	uint32_t _x[4] = { (uint32_t)(x.lo & 0xFFFF), (uint32_t)(x.lo >> 16), (uint32_t)(x.hi & 0xFFFF), (uint32_t)(x.hi >> 16) };
